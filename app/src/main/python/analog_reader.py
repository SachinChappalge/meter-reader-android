...
def calibrate_gauge(folder_path):
	'''
...
def getAnalogMeterReading(folder_path,min_angle, max_angle, min_value, max_value, units):
	#folder_path = input("Folder name inside this file's folder: ")
	#filename = input("Filename without extension .jpg: ")

	x, y, r = calibrate_gauge(folder_path)

    #feed an image (or frame) to get the current value, based on the calibration, by default uses same image as calibration
	img = cv2.imread(folder_path)
	return get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r)
